...
4. 修改 /etc/kubez/globals.yml 配置文件
    ...
    kube_release: 1.26.15

    修改 image_repository: "" 为本地镜像仓库
    image_repository: "本机IP:58001/pixiuio"

    修改 yum_baseurl: “” 为本地 yum 仓库地址
    yum_baseurl: "http://本机IP:58000/repository/pixiuio-centos"

    取消 calico 的注释，并将 no 修改为 yes
...
5. 执行如下命令，进行 `kubernetes` 的依赖安装
    ...
    执行完成之后，需要修改 containerd 的配置文件
    这个会提供文件，到时候直接修改 IP 即可，后续会更新的项目中
    
    修改 calico 文件
    vim /usr/share/kubez-ansible/ansible/roles/kubernetes/templates/kube-calico.yml.j2
    calico-node:v3.26.0
